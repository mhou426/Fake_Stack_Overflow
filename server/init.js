...
let tags = [];
let answers = [];
function userCreate(username, email,password,questions) {
    let user = new User({ user:username, email:email, password:password,questions:questions});
    return user.save();
  }
...
const populate = async () => {
  ...
  await userCreate('mango','andichen@gmail.com',pwHash,[q1,q2]);
  if(db) db.close();
  console.log('done');
}
